...
export default async function CasePage({
  ...
  const successCase = await payload.find({
  ...
  if (!successCase || successCase.docs.length <= 0) redirect("/cases");
  ...
  const shareLinks = generateShareLinks({
    title: successCase.docs[0].titulo,
    url: `${process.env.NEXT_PUBLIC_SITE_URL}/blog/${successCase.docs[0].slug!}`,
  });
      ...
      <section className="bg-white bg-[url(/images/img-bg-hero-blogpost.svg)] bg-contain bg-right bg-no-repeat pt-36 sm:pt-48">
        <div className="container">
            ...
            <div className="w-full md:w-[60%] lg:w-[70%]">
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="text-brand-dark-gray flex items-center gap-1 text-lg font-normal">
                  ...
                  {formatDistanceToNow(successCase.docs[0].createdAt, {
                    addSuffix: true,
                    locale: ptBR,
                  })}
                </div>
                <div className="text-brand-dark-gray flex items-center gap-1 text-lg font-normal">
                  ...
                  {readingTime({ richText: successCase.docs[0].conteudo })} min
                  de leitura
                </div>
              </div>
            </div>
            {successCase.docs[0].melhorias && (
              <div className="flex w-full flex-col items-end gap-5 md:w-[40%] lg:w-[30%]">
                {successCase.docs[0].melhorias.map((melhoria) => (
                  <div
                    key={melhoria.id}
                    className="w-full rounded-xl bg-black/30 p-6 md:max-w-[251px]"
                  >
                    {melhoria.porcentagem_melhoria && (
                      <p className="text-[28px] leading-[130%] font-bold text-white">
                        +{melhoria.porcentagem_melhoria}
                      </p>
                    )}
                    {melhoria.descricao_melhoria && (
          ...
          <div className="mt-10 w-full rounded-lg sm:h-[455px] lg:mt-20">
            <Image
              src={(successCase.docs[0].imagem_de_destaque as Media).url!}
              alt={(successCase.docs[0].imagem_de_destaque as Media).alt}
              width={1216}
              height={455}
      ...
      <section className="bg-white py-14">
        <div className="container">
          <RichText
            ...
            data={successCase.docs[0].conteudo}
          />
