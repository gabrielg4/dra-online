...
import type { Categoria, Media, User } from "../../../../../payload-types";

import { formatDistanceToNow } from "date-fns";
import { ptBR } from "date-fns/locale";
import { readingTime } from "@/utils/calculate-reading-time";
...
export default async function Blogpage({
  ...
  if (!post || post.docs.length <= 0) redirect("/blog");
  const shareLinks = generateShareLinks({
    title: post.docs[0].titulo,
    url: `${process.env.NEXT_PUBLIC_SITE_URL}/blog/${post.docs[0].slug!}`,
  });
      ...
      <section className="bg-white bg-[url(/images/img-bg-hero-blogpost.svg)] bg-contain bg-right bg-no-repeat pt-36 sm:pt-48">
        <div className="container">
          <div className="mb-5 flex items-center gap-4">
            <p className="bg-brand-main-green w-fit rounded-full px-4 py-2 text-sm leading-normal text-white">
              {(post.docs[0].categoria as Categoria).titulo}
            </p>
          </div>
          <h1 className="text-brand-dark-green mb-2 max-w-[768] text-[28px] leading-[110%] font-bold sm:mb-5 sm:text-[32] md:text-[40px] lg:text-[48px]">
            {post.docs[0].titulo}
          </h1>
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:gap-4">
            <p className="text-brand-dark-gray text-lg font-normal">
              {(post.docs[0].autor as User).nome}
            </p>
            <div className="text-brand-dark-gray flex items-center gap-1 text-lg font-normal">
              ...
              {formatDistanceToNow(post.docs[0].createdAt, {
                addSuffix: true,
                locale: ptBR,
              })}
            </div>
            <div className="text-brand-dark-gray flex items-center gap-1 text-lg font-normal">
              ...
              {readingTime({ richText: post.docs[0].conteudo })} min de leitura
            </div>
          </div>

          <div className="bg-brand-main-green mt-10 w-full rounded-lg sm:h-[455px] lg:mt-20">
            <Image
              src={(post.docs[0].imagem_de_destaque as Media).url!}
              alt={(post.docs[0].imagem_de_destaque as Media).alt}
              width={1216}
              height={455}
