...
export const NaMidiaCard = ({ isActive, article }: NaMidiaCardProps) => {
  return (
    <div
      className={`wheel-card border-brand-light-green h-[343px] w-full max-w-[290px] rounded-2xl border bg-white/10 backdrop-blur-[12px] max-sm:mx-auto`}
    >
      <div
        className={cn(
      ...
      >
        ...
        {isActive && (
          <Button
            variant="outline"
            className="border-brand-light-green group hover:bg-brand-light-green h-auto cursor-pointer rounded-full border bg-transparent !p-0 duration-300"
          >
            <Link
              href={article.link_artigo}
              target="_blank"
              prefetch={false}
              className="text-brand-light-green px-6 py-2 duration-300 group-hover:text-white"
            >
              Ler mais
            </Link>
          </Button>
