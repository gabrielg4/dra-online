...
export const BlogCarousel = () => {
  ...
  return (
    ...
    >
      <CarouselContent className="-ml-8 pt-3">
        {placeholderBlogposts.map((BlogpostCard, index) => (
          <CarouselItem
            key={index + 1}
            className={cn(
              "basis-full pl-8 transition-transform duration-300 sm:basis-1/2 md:basis-1/3",
      ...
      <CarouselNext className="hover:text-brand-light-green hidden h-10 w-10 cursor-pointer border-0 bg-transparent text-white hover:bg-transparent sm:flex md:-right-12" />
      <CarouselPrevious className="hover:text-brand-light-green hidden h-10 w-10 cursor-pointer border-0 bg-transparent text-white hover:bg-transparent sm:flex" />
